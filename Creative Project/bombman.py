...
#AG: Declaring base mod list. 0: danger_curve, 1,2,3: bomb_ratio_curve, 4,5: time_impact_curve, 6: Bomb_throw_curve
mod = [1.0001,.3,.2,.3,4,.45, -10]
f = open("danger_map.txt", 'w')
g = open("danger_values.txt", 'w')
h = open("ratings.txt", 'w')
i2 = open("ratio.txt", 'w')
j2 = open("time_alive.txt", 'w')
k2 = open("map_data.txt", 'w')
l = open("ai_map_data.txt",'w')
m = open("movement.txt",'w')
n = open("bomb_tile_indices.txt",'w')
o = open("movement_queue.txt",'w')
p = open("bomb_queue.txt",'w')
q2 = open("distance_formula.txt",'w')
...
class MapTile(object):
  ...
  def __init__(self, coordinates):
    ...
    #AG: Adding danger_value and player
  ...
  def update_danger_map(self):
    for j in range(Map.MAP_HEIGHT):  # reset
      for i in range(Map.MAP_WIDTH):
        ...
        #AG: Made danger_map include items
        elif tile.item is not None:
          if tile.item is not 'disease':
            self.danger_map[j][i] = 8000
          else:
            self.danger_map[j][i] = 0
        else:
    ...
    for bomb in self.bombs:
      ...
      print("Bomb_tile: "+str(bomb_tile))
      if bomb_tile[1] > 10:
        bomb_tile = (bomb_tile[0],10)
        print("Fixed bomb tile")
      if bomb_tile[0] > 14:
        bomb_tile = (14,bomb_tile[1])
        print("Fixed bomb tile")
      print("Bomb_tile: "+str(bomb_tile))
...
#        time_until_explosion = 100
      n.write('Looking up: ['+str(bomb_tile[1])+']['+str(bomb_tile[0])+']\n') 
      ...
      for i in range(Map.MAP_HEIGHT):  # reset
        f.write('\n')
        for j in range(Map.MAP_WIDTH):
          if self.danger_map[i][j] < 5000:
            self.ai_map[i][j].danger_value = self.danger_map[i][j]
            if self.danger_map[i][j] == 0:
              self.ai_map[i][j].danger_value = None
          else:
            self.ai_map[i][j].danger_value = None
          f.write(str(self.danger_map[i][j]))
          if self.danger_map[i][j] < 1000:
            if self.danger_map[i][j] < 10:
              f.write("    ")
            elif self.danger_map[i][j] < 100:
              f.write("   ")
            else:
              f.write("  ")
          else:
  ...
  def bomb_explodes(self, bomb):
    ...
    if bomb_position[1] > 10:
      bomb_position = (bomb_position[0],10)
    if bomb_position[0] > 14:
      bomb_position = (14,bomb_position[1])
  ...
  def update(self, dt, immortal_player_numbers=[]):
    ...
    for player in self.players:
      ...
      if player.get_state() != Player.STATE_IN_AIR and player.get_state != Player.STATE_TELEPORTING and (self.tile_has_flame(player_tile.coordinates) or self.tile_has_lava(player_tile.coordinates)):
        ...
        if not (player.get_number() in immortal_player_numbers):
          ...
          #AG: Should output when players die
          with open("time_alive.txt", "a") as j2:
            j2.write(str(player.get_number())+" died at "+str(self.time_from_start)+'\n')
          #CH: Adds time of death to data array
          global data
          current = genetic.Data()
          current.ai_number = player.get_number()
          current.time_alive = self.time_from_start
          data.append(current)
          player.kill(self)
          continue
    ...
    #AG: Outputting map_data
    for j in range(Map.MAP_HEIGHT):
      k2.write('\n')
      for i in range(Map.MAP_WIDTH):
        del self.ai_map[j][i].player[:]
        isPlayer = False
        tile = self.tiles[j][i]
        for player in self.players:
          player_tile_position = player.get_tile_position()
          player_tile = self.tiles[player_tile_position[1]][player_tile_position[0]]
          if player_tile is tile and not player.is_dead():
            k2.write('P'+str(player.get_number())+' ')
            #AG: Need to change this to a list for if there are multiple players on one space
            self.ai_map[j][i].player.append(player.get_number())
            isPlayer = True
        if isPlayer is False:
          k2.write(str(tile.kind)+'  ')
        self.ai_map[j][i].kind = tile.kind
    k2.write('\n')
    
    #AG: Outputting ai_map_data
    for j in range(Map.MAP_HEIGHT):
      l.write('\n')
      for i in range(Map.MAP_WIDTH):
        curr = self.ai_map[j][i]
        if len(curr.player) > 0 and curr.danger_value is None:
            l.write("P"+str(curr.player[0]))
            if len(curr.player) > 1:
              l.write("+  ")
            else:
              l.write("   ")          
        elif curr.danger_value is not None and len(curr.player) == 0:
          l.write(str(curr.danger_value))
          if curr.danger_value < 1000:
            if curr.danger_value < 10:
              l.write("    ")
            elif curr.danger_value < 100:
              l.write("   ")
            else:
              l.write("  ")
          else:
            ...
            if len(curr.player) > 1:
              ...
              l.write("!  ") 
        elif curr.item is not None and curr.kind != MapTile.TILE_BLOCK:
          l.write("item ")
        else:
          l.write('t'+str(curr.kind)+'   ')
        #AG: add check for teleporter later
    l.write('\n')
    ...
    if self.state == Map.STATE_WAITING_TO_PLAY:  
      if self.time_from_start >= self.start_game_at:
        ...
        #AG: Opening timing file when game starts
        j2 = open("time_alive.txt",'a')
...
class AboutMenu(Menu):
  def __init__(self,sound_player):
    ...
    self.text = ("^#2E44BFBombman^#FFFFFF - an open-source Atomic Bomberman clone, ^#4EF259version " + Game.VERSION_STR + "\n"
                 ...
                 "Featuring new AI perfected by Andrew Gwinner, Nicholas Ho, and Chris Hynuh.\n"
                 "There I said perfected, you happy Nick :P? Works on Python 3.2 now.")
...
class Renderer(object):
    ...
    if self.preview_map_name != map_filename:
      ...
      with open(os.path.join(Game.MAP_PATH,map_filename)) as map_file:
        ...
        for player_index in range(len(starting_positions)):
          draw_position = (int(starting_positions[int(player_index)][0]) * int(tile_size) + int(tile_half_size),int(starting_positions[int(player_index)][1]) * int(tile_size) + int(tile_half_size))
           
          pygame.draw.rect(self.preview_map_image,tile_color,pygame.Rect(pos_x,pos_y,tile_size,tile_size))
          pygame.draw.circle(self.preview_map_image,Renderer.COLOR_RGB_VALUES[int(player_index)],draw_position,int(tile_half_size))
  ...
  def render_map(self, map_to_render):
    ...
    for line in tiles:
      ...
      while True:                  # render players and bombs in the current line 
        ...
        if isinstance(object_to_render,Player):    # <= not very nice, maybe fix this later
          ...
          if object_to_render.is_in_air():
            ...
            player_image = self.player_images[int(color_index)]["down"]
            ...
            if animation_frame == 0:
              image_to_render = self.player_images[int(color_index)]["up"]  
            elif animation_frame == 1:
              image_to_render = self.player_images[int(color_index)]["right"]  
            elif animation_frame == 2:
              image_to_render = self.player_images[int(color_index)]["down"]  
            else:
              image_to_render = self.player_images[int(color_index)]["left"]  
            ...
            if object_to_render.get_state() == Player.STATE_IDLE_UP or object_to_render.get_state() == Player.STATE_WALKING_UP:
              image_to_render = self.player_images[int(color_index)][helper_string + "up"]
            elif object_to_render.get_state() == Player.STATE_IDLE_RIGHT or object_to_render.get_state() == Player.STATE_WALKING_RIGHT:
              image_to_render = self.player_images[int(color_index)][helper_string + "right"]
            elif object_to_render.get_state() == Player.STATE_IDLE_DOWN or object_to_render.get_state() == Player.STATE_WALKING_DOWN:
              image_to_render = self.player_images[int(color_index)][helper_string + "down"]
            else:      # left
              image_to_render = self.player_images[int(color_index)][helper_string + "left"]
          else:
            if object_to_render.get_state() == Player.STATE_IDLE_UP:
              ...
              image_to_render = self.player_images[int(color_index)]["walk up"][int(animation_frame)]
            elif object_to_render.get_state() == Player.STATE_WALKING_RIGHT:
              image_to_render = self.player_images[int(color_index)]["walk right"][int(animation_frame)]
            elif object_to_render.get_state() == Player.STATE_WALKING_DOWN:
              image_to_render = self.player_images[int(color_index)]["walk down"][int(animation_frame)]
            else: # Player.STATE_WALKING_LEFT
              image_to_render = self.player_images[int(color_index)]["walk left"][int(animation_frame)]
        
          if object_to_render.get_disease() != Player.DISEASE_NONE:
            overlay_images.append(self.other_images["disease"][int(animation_frame % 2)]) 
          profiler.measure_stop("map rend. player")
        else:    # bomb
          ...
          image_to_render = self.bomb_images[int(animation_frame)]
      ...
      for tile in reversed(line):             # render tiles in the current line
        ...
        if len(tile.flames) != 0: # there is at least one flame - draw it
          ...
          result.blit(self.flame_images[int(flame_animation_frame)][sprite_name],(x,y))
    ...
    if map_to_render.get_state() == Map.STATE_WAITING_TO_PLAY:
      ...
      countdown_image = self.gui_images["countdown"][math.ceil(countdown_image_index)]
...
class AI(object):
  ...
  def __init__(self, player, game_map):
    ...
    self.move = q.PriorityQueue()
    #AG: Added these so AI will know what it is working towards
    self.itemFound = False
    self.playerFound = False 
    self.trampFound = False
    #AG: Added these to improve bomb placement and reaction to danger
    self.total_walkable = 0
    self.walkable_neighbors = 0
  ...
  def get_tile_neighbors(self, tile_coordinates):
    x = tile_coordinates[0]
    y = tile_coordinates[1]
    neighbors = []
    if self.game_map.tile_is_withing_map((x+1,y)):
      neighbors.append((x+1,y))
    if self.game_map.tile_is_withing_map((x-1,y)):
      neighbors.append((x-1,y))
    if self.game_map.tile_is_withing_map((x,y+1)):
  ...
  #AG: Gives distance between 2 points. Now is Manhattan  
  def distance_formula(self,p1,p2):
    q2.write("p1: "+str(p1)+", p2: "+str(p2)+"\n")
    x1,y1 = p1
    x2,y2 = p2
    return abs(x2-x1) + abs(y2-y1)                                                                        
  
  #AG: BFS to find nearby objects                                                                   
  def choose_target(self):
 
    #AG: If there are two players left, make them go towards each other
    players = self.game_map.players
    dead_count = 0
    for player in players:
      if player.is_dead():
        dead_count +=1
    if len(players)-dead_count == 2:
      for player in players:
        if player.get_number() == self.player.get_number():
          continue
        else:
    ...
    graph = self.game_map.ai_map
#    for i in range(Map.MAP_HEIGHT):
#      for j in range(Map.MAP_WIDTH):
#        print("("+str(i)+","+str(j)+"): "+str(self.game_map.ai_map[i][j].kind))
    self.itemFound = False
    self.playerFound = False
    self.trampFound = False
    frontier = []
    ...
    while len(frontier) > 0:
      ...
      spot = graph[current[0]][current[1]]
      if spot.item is not None and spot.kind is not 1:
        destination = current
        self.itemFound = True
        break
      if len(spot.player) > 1:
        SameAsPlayer = False
        for dude in spot.player:
          if dude == self.player.get_number():
            SameAsPlayer = True
        if SameAsPlayer is False:
          destination = current
          self.playerFound = True
          break
        if spot.special_object is not None and spot.kind == 1:
          self.trampFound = True
          destination = current
          break
#      print("Looking at ",current)
      for new in self.get_tile_neighbors(current):
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        if new not in visited and self.distance_formula(start,new) <= 5 and new[0] <= 10 and new[1] <= 14:
#          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
          heappush(frontier, new)
          visited[new] = current
    #players = self.game_map.gets_players()
    m.write('Player '+str(self.player.get_number())+' @'+str(self.player.get_position())+'found ')
    if self.itemFound is True:
      m.write('item @'+str(destination)+'\n')
      return destination
    elif self.playerFound is True:
      m.write('enemy @'+str(destination)+'\n')
      return destination
    else:
      m.write("nothing\n")
      return None
  ...
  #AG: Allows for pathfinding between teleports    
  def get_tile_neighbors2(self, tile_coordinates, came_from):
      
    x = tile_coordinates[0]
    y = tile_coordinates[1]
    neighbors = []
    #print(tile_coordinates)  
    if self.game_map.tile_has_teleport:
      teleports_to = self.game_map.tiles[tile_coordinates[1]][tile_coordinates[0]].destination_teleport
      if came_from[tile_coordinates] is not teleports_to:
        neighbors.append(teleports_to)
        return neighbors
  ...
  #AG: A* to find path to that object
  def path_to_target(self, destination):
    graph = self.game_map.ai_map
    start = self.player.get_position()
...
#    print(start)
    ...
    while len(frontier) > 0:
      ...
      if current[1] == destination:
        ...
        break
#      print("Looking at ",current)
      for new in self.get_tile_neighbors2(current[1],came_from):
#        print("New: ",new)
        if new is None:
          continue
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        spot = graph[new[1]][new[0]]
        if not self.game_map.tile_has_teleport(new) or not self.game_map.tile_is_walkable(new):
          continue
        if spot.danger_value is not None:
          if spot.danger_value < 500:
            continue                 
        if new[0] <= 10 and new[1] <= 14:
#          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
          new_cost = cost_so_far[current[1]] + 1 #AG: On RedBlob this is graph.cost(current, next) but distance should always be one
          if spot.danger_value is not None:
            new_cost += (3000 - spot.danger_value)
          if new not in cost_so_far or new_cost < cost_so_far[new]:
            cost_so_far[new] = new_cost
            priority = new_cost + self.distance_formula(new,destination)
            heappush(frontier, (priority, new))
            came_from[new] = current

    if pathFound == True:
    #Red Blob Code for retrieving path
        current = destination
        path = [current]
        while current != start:
            junk, current = came_from[current]
            path.append(current)
        path.reverse()
        m.write(str(path))
        return path[0]
  ...
  #AG: Normalizing function
  def normalize_bomb_danger(self,danger):
    newvalue = danger/3000
    return newvalue
                  
  #AG: Utility curve 
  def bomb_danger_curve(self,danger):
    #AG: Base curve is y=(1/1.001)^x
    if danger is None:
      danger = 5000
    y = pow((1/mod[0]),danger)
    return y
  ...
  def rate_tile(self, tile_coordinates):
    ...
    if danger != 5000:
      normy_danger = self.normalize_bomb_danger(danger)
      curve_score = self.bomb_danger_curve(danger)
      #AG: Output
      g.write('Danger: ' + str(danger)+ ' ')
      g.write('Norm: ' + str(normy_danger)+ ' ')
      g.write('Curve Score: ' + str(curve_score)+ ' ')
      
    score = self.bomb_danger_curve(danger)
    score = math.ceil(100 - score*100)
    ...
    if self.game_map.tile_has_bomb(tile_coordinates):
      if not self.player.can_box():
        score -= 5
    if danger != 5000:
      str_score = 'Final Score: '+str(score) + ' '
      g.write(str_score)
      g.write('\n')    
  ...
  #NH: check if there are too many walls
  def too_many_walls(self):
    neighbour_tiles = self.player.get_neighbour_tile_coordinates()
    too_many = False
    count=0
    for tile_coordinates in neighbour_tiles:
    ...
    for player in self.game_map.get_players():
      ...
      if abs(current_position[0] - player_position[0]) <= 1 and abs(current_position[1] - player_position[1]) <= 1:
        if player.is_enemy(self.player):
          enemies += 1
#        else:                AG: I commented out the code that updates allies because we won't need it
#          allies += 1
  ...
  #AG: for BFS for walkable from position                                                                         
  def get_tile_neighbors3(self, tile_coordinates):
    x = tile_coordinates[0]
    y = tile_coordinates[1]
    neighbors = []
    if self.game_map.tile_is_withing_map((y,x+1)) and self.game_map.tile_is_walkable:
      neighbors.append((x+1,y))
    if self.game_map.tile_is_withing_map((y,x-1)) and self.game_map.tile_is_walkable:
      neighbors.append((x-1,y))
    if self.game_map.tile_is_withing_map((y+1,x)) and self.game_map.tile_is_walkable:
      neighbors.append((x,y+1))
    if self.game_map.tile_is_withing_map((y-1,x)) and self.game_map.tile_is_walkable:
      neighbors.append((x,y-1))
#    print(neighbors)
  ...
  def get_tile_neighbors3(self, tile_coordinates):
    ...
    y = tile_coordinates[1]
    neighbors = []
    if self.game_map.tile_is_withing_map((y,x+1)) and self.game_map.tiles[y][x+1].kind == 0:
      neighbors.append((x+1,y))
    if self.game_map.tile_is_withing_map((y,x-1)) and self.game_map.tiles[y][x-1].kind == 0:
      neighbors.append((x-1,y))
    if self.game_map.tile_is_withing_map((y+1,x)) and self.game_map.tiles[y+1][x].kind == 0:
      neighbors.append((x,y+1))
    if self.game_map.tile_is_withing_map((y-1,x)) and self.game_map.tiles[y-1][x].kind == 0:
      neighbors.append((x,y-1))
    print(neighbors)
  ...
  #AG: BFS from player searching for safety, then directs to that safety
  def get_to_safety(self):
    graph = self.game_map.ai_map
...
#    print(start)
    ...
    while len(frontier) > 0:
      current = heappop(frontier)
      #      print("Looking at ",current)
      for new in self.get_tile_neighbors3(current):
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        spot = graph[new[0]][new[1]]
        if spot.danger_value is None:
          heappush(safety,(self.distance_formula(start,new),current))
        if new not in visited and new[0] <= 10 and new[1] <= 14:
#        print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
          heappush(frontier, new)
          visited[new] = current
    if len(safety) < 1:
      ...
      destination = heappop(safety)
#      print("Safety: "+str(destination[1]))
      return self.path_to_target(destination[1])
              
  #AG: BFS updating safe_tiles around player
  def update_surrounding_safe_tiles(self):
    graph = self.game_map.ai_map
...
#        print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
          heappush(frontier, new)
          visited[new] = current
    print("Walkable neighbors: "+str(self.walkable_neighbors)+" and Total walkable: "+str(self.total_walkable))

    
  def smart_to_bomb_curve(self,ratio):
    # y=(x)^{0.5}
#    n.write("smart_to_bomb_curve:\n")
    if self.total_walkable == 0:
#      print("Total walkable was 0")
      return 1
    #AG: Added 5 - neighbors so it is disencouraging putting a bomb when low on escapable tiles
    y = pow(((5 - self.walkable_neighbors)/self.total_walkable),ratio)
#    n.write("(walkable_neighbors: "+str(self.walkable_neighbors)+"/ total_walkable: "+str(self.total_walkable)+") to the "+str(mod[4])+" yields: "+str(y)+'\n')
    return y
    
  def bomb_throw_curve(self):
      y = self.walkable_neighbors/4
      y = mod[6] * (y - 0.5)
      y = 2/(2 + pow(math.e,y))
           
      return y
  
  #AG: Normalizes time_since_hasn't_moved
  def normalize_time_value(self):
    if self.game_map.get_map_time() == 0:
      return 1
    else:
      normalized = self.didnt_move_since/self.game_map.get_map_time()
      if normalized == 1:
        return .05
      else:
        return normalized
    
  #AG: Curve for how much time since haven't moved matters
  def time_impact_curve(self):
    #Base curve: y = (1/(1+e^(4x)))+.45
    time_value = 1 - self.normalize_time_value()
    y = 1 / (1 + pow(math.e, mod[5]*time_value))
    y += mod[5]
    #AG: To disencourage randomness when near danger
    if len(self.game_map.bombs) > 0:
      y *= (5-self.walkable_neighbors)
    return y   
        
    
  def play(self):
    #CH: Custom  mod for each AI
    global mod
    mod = pop.generate_mods()[self.player.get_number()]
    self.update_surrounding_safe_tiles()
    #AG: This is for identifying each player's thoughts
    strIdentity = str(self.player.get_number())
    o.write("Player #"+strIdentity+'\n')
    p.write("Player #"+strIdentity+'\n')
    
    #AG: Normalizing function for curve output
    def norm_ratio_bomb_curve_output(y):
      maxInRange = ratio_bomb_curve(1)
      minInRange = ratio_bomb_curve(0)
      newvalue = (y - minInRange)/(maxInRange - minInRange)
      return newvalue
        
    def ratio_bomb_curve(ratio):
      #AG: base curve is y=(.2*(-(x-.3)^{(1/3)}))+.3
      if ratio-mod[1] > 0:
        y = -1*pow((ratio-mod[1]),(1/3))
      else:
        y = pow(abs(ratio-mod[1]),(1/3))
      y *= mod[2]
      y += mod[3]
      return y

    #AG: Returns reverse probability from curve      
    def use_curve(y):
      #output = 1-y
      norm_out = norm_ratio_bomb_curve_output(y)
      norm_out = 1 - norm_out
      return norm_out
    ...
    if target is not None:
      direction = self.path_to_target(target)
      
    if direction is not None:
      if current_tile[0] - direction[0] == 0:
        if current_tile[1] - direction[1] == 1:
          chosen_movement_action = PlayerKeyMaps.ACTION_UP  
        else: #current_tile[1] - direction[1] == -1:
          chosen_movement_action = PlayerKeyMaps.ACTION_DOWN 
      else: #current_tile[1] - direction[1] == 0:
        if current_tile[0] - direction[0] == 1:
          chosen_movement_action = PlayerKeyMaps.ACTION_LEFT 
        else: #current_tile[0] - direction[0] == -1:
          chosen_movement_action = PlayerKeyMaps.ACTION_RIGHT
      print('in a*')
      self.move.put((999999 ,chosen_movement_action))
      o.write("in a*"+"("+str(999999)+","+str(chosen_movement_action)+")\n")
    ...
    move_priority = use_curve(self.game_map.get_danger_value(current_tile)/3000) 
    trapped = self.is_trapped()
    escape_direction_ratings = self.rate_bomb_escape_directions(current_tile)
    
    #AG: text addition
    h.write(strIdentity + " is at " + str(current_tile) + ". Trapped: "+str(trapped)+". Best escape: "+str(escape_direction_ratings)+"\n")
    ...
    if trapped:
      ...
      #NH: need a heuristic for this one
      h.write("Movement choice for trapped: random\n")
      ...
      print('in trapped')
      self.move.put((use_curve(self.game_map.get_danger_value(current_tile)) ,chosen_movement_action))
      o.write("in trapped"+"("+str(use_curve(self.game_map.get_danger_value(current_tile)))+","+str(chosen_movement_action)+")\n")
    
      
    #NH: kinda hard to explain what im trying to do
    else: #elif self.game_map.tile_has_bomb(current_tile):
      ...
      if max(escape_direction_ratings) > 0:
          if escape_priority > 3000:
            o.write("Escape priority: "+str(escape_priority))
            escape_priority = use_curve(1)
          else:
            #AG: Added check in case bomb is not on danger_map yet
            if self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value is None:
              escape_priority = 10
            else:              
              escape_priority = 1 - self.bomb_danger_curve(self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value)  #- best_rating
          self.move.put((escape_priority-1,best_action))
          print(escape_priority)
          #print(escape_priority)

      #NH: need a way to use the curve for this but placeholder
      if self.game_map.tile_has_bomb(current_tile):
          escape_priority -= 1000000 
      chosen_movement_action = best_action
      #self.move.put((escape_priority-1,chosen_movement_action))
      #print(escape_priority)
      #print(best_action)
      #print('in escape')
      #print(escape_direction_ratings)
      direction = self.get_to_safety()
      if direction is not None:
        if current_tile[0] - direction[0] == 0:
          if current_tile[1] - direction[1] == 1:
            chosen_movement_action = PlayerKeyMaps.ACTION_UP  
          ...
          if current_tile[0] - direction[0] == 1:
            ...
            chosen_movement_action = PlayerKeyMaps.ACTION_RIGHT
        self.move.put((0,chosen_movement_action))
      #self.move.put((escape_priority,chosen_movement_action))
      o.write("tile has bomb"+"("+str(escape_priority)+","+str(chosen_movement_action)+")\n")
      
      h.write("Movement choice for on bomb: "+best_direction+"\n")
    #else:             # not standing on a bomb
      #NH: This entire part should be changed
      # should I not move?
      ...
      h.write("Movement choice for not on bomb: "+str(chosen_movement_action)+". 0 is up, 1 is right, 2 is down, and 3 is left\n")
    ...
    if chosen_movement_action != None:
      ...
      #NH: Kinda a place holder fo andrew, since i need the pathfinding for this one
      #print('in chosen move')
      #print(use_curve(self.rate_tile(current_tile)))
      #self.move.put((use_curve(self.rate_tile(current_tile)),chosen_movement_action))
      self.move.put((-98,chosen_movement_action))
      o.write("Somewhat random movement"+"("+str(-98)+","+str(chosen_movement_action)+")\n")

      self.didnt_move_since = self.game_map.get_map_time()

    else:#if self.game_map.get_map_time() - self.didnt_move_since > 10000:   # didn't move for 10 seconds or more => force move
      #NH: Chance to not move
      no_move_priority = -99 #use_curve(self.game_map.get_danger_value(current_tile)/3000)
      self.move.put((no_move_priority,math.pi))
      o.write("not move"+"("+str(no_move_priority)+","+str(math.pi)+")\n")
    
      #NH: Yet again, need a heuristic for this
      chosen_movement_action = random.choice((PlayerKeyMaps.ACTION_UP,PlayerKeyMaps.ACTION_RIGHT,PlayerKeyMaps.ACTION_DOWN,PlayerKeyMaps.ACTION_LEFT))
      move_priority = self.time_impact_curve()  #-  use_curve(self.game_map.get_danger_value(current_tile)) 
      #self.outputs.append((self.player.get_number(),chosen_movement_action))
      print('in force move')

      self.move.put((move_priority,chosen_movement_action))
      o.write("force move"+"("+str(move_priority)+","+str(chosen_movement_action)+")\n")
      strchosen = str(chosen_movement_action)
      h.write("Movement forced to: "+strchosen+". 1 is up, 2 is right, 3 is down, and 4 is left\n")

    # bomb decisions
    
    bomb_laid = False
    bomb_priority = use_curve(self.game_map.get_danger_value(current_tile)/3000) 
    if self.game_map.tile_has_bomb(current_tile):
      # should I throw?
      
      if self.player.can_throw(): #and max(escape_direction_ratings) == 0:
        #NH: Arbitrary 10 to increase chance of throwing rather that running for testing. Bomb throw should have its own curve
        bomb_priority = self.bomb_throw_curve()# ( use_curve(self.game_map.get_danger_value(current_tile)/3000)) - 10 
        #self.outputs.append((self.player.get_number(),PlayerKeyMaps.ACTION_BOMB_DOUBLE))
        print('in throw')
        self.queue.put((bomb_priority,PlayerKeyMaps.ACTION_BOMB_DOUBLE))
        p.write("in throw"+"("+str(bomb_priority)+","+str(PlayerKeyMaps.ACTION_BOMB_DOUBLE)+")\n")
        h.write("Bomb choice: throw double bombs if trapped\n")
      ...
      if players_near[0] > 0 and players_near[1] == 0:  # enemy nearby and no ally nearby
        ...
        '''
        strrat = str(block_tile_ratio)
        i2.write("Block/Tile Ratio: "+strrat)
        
        chance2 = ratio_bomb_curve(block_tile_ratio)
        strChance = str(chance2)
        i2.write(". Curve Chance: "+strChance+'. ')
        strNormy = str(norm_ratio_bomb_curve_output(chance2))
        i2.write("Normalized Output: "+strNormy+'\n')
      ...
      '''
        i2.write("Block/Tile Ratio: "+str(block_tile_ratio))       
        chance = ratio_bomb_curve(block_tile_ratio)
        i2.write(". Curve Chance: "+str(chance)+'. ')
        i2.write("Normalized Output: "+str(norm_ratio_bomb_curve_output(chance))+'\n')
        chance_to_put_bomb = use_curve(chance)
        
        other_chance = self.smart_to_bomb_curve(chance)
        #other_chance2 = self.smart_to_bomb_curve2(chance_to_put_bomb)
        
        #AG: Adjusting to work with non-utility based code
        chance_to_put_bomb = int(chance_to_put_bomb*100)
#        print("Chance to put bomb before: "+str(chance_to_put_bomb))
        chance_to_put_bomb = int(other_chance*100)
      ...
      if number_of_block_neighbours == 1:
        ...
        chance_to_put_bomb = 2
      #NH: possible get rid of random element
      '''
      ...
      if do_lay_bomb:
        ...
        #Nh: temp; make a wall curve later
        if self.too_many_walls():
          self.queue.put((-1,math.pi))
          p.write("too many walls"+"("+str(-1)+","+str(math.pi)+")\n")
        
        if random.randint(0,2) == 0 and self.should_lay_multibomb(chosen_movement_action):  # lay a single bomb or multibomb?
          #self.outputs.append((self.player.get_number(),PlayerKeyMaps.ACTION_BOMB_DOUBLE))
          #NH: temp for temting purposes
          print('in bomb double')
          self.queue.put((0,PlayerKeyMaps.ACTION_BOMB_DOUBLE))
          p.write("in bomb double"+"("+str(0)+","+str(PlayerKeyMaps.ACTION_BOMB_DOUBLE)+")\n")
          
          h.write("Bomb choice: laying double bomb\n")
        else:
          #self.outputs.append((self.player.get_number(),PlayerKeyMaps.ACTION_BOMB))
          #NH: same
          print('in bomb')
          self.queue.put((0,PlayerKeyMaps.ACTION_BOMB))
          p.write("in bomb"+"("+str(0)+","+str(PlayerKeyMaps.ACTION_BOMB)+")\n")
          h.write("Bomb choice: laying single bomb\n")
    ...
    if self.player.can_box() and not self.player.detonator_is_active():
      if trapped or self.game_map.tile_has_bomb(self.player.get_forward_tile_position()):
        #NH: arbitrary for testing
        box_choose=random.randint(0,20)
        box_priority = (use_curve(self.game_map.get_danger_value(current_tile)/3000) ) - box_choose
        print('in box')
        self.queue.put((box_priority,PlayerKeyMaps.ACTION_SPECIAL))
        p.write("in box"+"("+str(box_priority)+","+str(PlayerKeyMaps.ACTION_SPECIAL)+")\n")
        #self.outputs.append((self.player.get_number(),PlayerKeyMaps.ACTION_SPECIAL))
        h.write("Bomb choice: boxing\n")
        
    if bomb_laid:   # if bomb was laid, the outputs must be recomputed fast in order to prevent laying bombs to other tiles
    #AG: Commented out code that kept AI from running each frame. Makes avatars glitch but runs fine
    ...
    if self.player.detonator_is_active():
       ...
       if random.randint(0,2) == 0 :
        det_priority = use_curve(self.game_map.get_danger_value(current_tile)/3000)
        print('in det')
        self.queue.put((det_priority,PlayerKeyMaps.ACTION_SPECIAL))
        p.write("in det"+"("+str(det_priority)+","+str(PlayerKeyMaps.ACTION_SPECIAL)+")\n")
        #self.outputs.append((self.player.get_number(),PlayerKeyMaps.ACTION_SPECIAL))
        h.write("Detonate\n")
    '''    
    h.write("Outputs: ")
    ...
    if self.queue.empty():
        if self.move.empty():
            return []
        just_move = self.move.get()
        if just_move[1] == math.pi:
            self.outputs.append((self.player.get_number(),None))
            o.write("FINAL MOVE CHOICE:"+"("+str(None)+")\n")
        else:
            self.outputs.append((self.player.get_number(),just_move[1]))
            o.write("FINAL MOVE CHOICE:"+"("+str(just_move)+")\n")
        return self.outputs
    if self.move.empty():
        just_action = self.queue.get()
        if just_action[1] == math.pi:
            o.write("FINAL MOVE CHOICE:"+"("+str(None)+")\n")
            self.outputs.append((self.player.get_number(),None))
        else:
            o.write("FINAL MOVE CHOICE:"+"("+str(just_move)+")\n")
            self.outputs.append((self.player.get_number(),just_action[1]))
        return self.outputs
        
    chosen_move1 = self.move.get()
    o.write("FINAL MOVE CHOICE:"+"("+str(chosen_move1)+")\n")
    chosen_action1 = self.queue.get()
    p.write("FINAL MOVE CHOICE:"+"("+str(chosen_action1)+")\n")
    chosen_move = chosen_move1[1]
    chosen_action = chosen_action1[1]
    if chosen_move == math.pi:
        chosen_move = None
    if chosen_action == math.pi:
        chosen_action = None
    print(chosen_move1)
    self.outputs.append((self.player.get_number(),chosen_move))
    self.outputs.append((self.player.get_number(),chosen_action))
    return self.outputs
...
#    j2.open("time_alive.txt",'w')
    ...
    while True:     # main loop
      ...
      if self.state == Game.STATE_PLAYING:
        #CH: Creates initial population based on number of AI
        if self.game_number == 1:
          global pop
          pop = genetic.Population(len(self.ais))
        ...
        if self.game_map.get_state() == Map.STATE_GAME_OVER:
          ...
          if self.game_number > self.play_setup.get_number_of_games():
            #Returns best ai
            global data
            winners = genetic.fitness(data)
            best = pop.generate_mods()[winners[0]]
            ...
            #CH: When round ends perform genetic algorithm
            global data
            global pop
            winners = genetic.fitness(data)
            new_pop = genetic.crossover(winners, pop)
            pop = new_pop
            data = []
